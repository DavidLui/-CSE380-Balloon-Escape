...
ScreenGUI::ScreenGUI()
{
	...
	lifeStatusImages = new list<LifeStatus*>();
	screenName = NULL;
}
...
void ScreenGUI::addLifeStatus(LifeStatus *imageToAdd) 
{
	lifeStatusImages->push_back(imageToAdd);
}
/*
...
*/
void ScreenGUI::addRenderItemsToRenderList(RenderList *renderList, unsigned int playerLife, unsigned int lives)
{
	...
	list<LifeStatus*>::iterator lifeImagesIterator;
	lifeImagesIterator = lifeStatusImages->begin();
	unsigned int index;
	while (lifeImagesIterator != lifeStatusImages->end())
	{
		LifeStatus *image = (*lifeImagesIterator);
		...
		renderList->addRenderItem(	image->statusID[index],
									image->x,
									image->y,
									image->z,
									image->alpha,
