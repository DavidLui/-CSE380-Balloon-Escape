...
void GameStateManager::About()
{
	currentGameState = GS_ABOUT;
}
void GameStateManager::unloadingLevel()
{
	currentGameState = GS_UNLOADING_LEVEL;
}
/*
...
*/
void GameStateManager::addLevel(wstring levelToAddName, wstring levelToAddFileDir)
{
	levelNames.push_back(levelToAddName);
	levelDirs.push_back(levelToAddFileDir);
}
...
void GameStateManager::loadLevel(Game *game, unsigned int initLevel)
{
	if ((initLevel > NO_LEVEL_LOADED) && (initLevel <= levelNames.size()))
	{
		...
		wstring fileToLoad = levelNames[currentLevel];
		wstring fileDir = levelDirs[currentLevel];
		GameDataLoader *dataLoader = game->getDataLoader();
		dataLoader->loadWorld(game, fileDir, fileToLoad);
	}
}
...
void GameStateManager::setCurrentGameState(GameState state)
{
